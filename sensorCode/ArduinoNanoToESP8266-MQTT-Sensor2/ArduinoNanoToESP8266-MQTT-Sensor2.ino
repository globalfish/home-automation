...
long doorSamplerTimer;
long heartbeatTimer;
long distance = 0;
char jsonObj1[256];

float temperature,prevTemperature = 0.0, lightLevel, prevLightLevel = 0.0;
float lightLevel2, prevLightLevel2 = 0.0;
int motionLevel, prevMotionLevel = 0;
int doorClosedState = 1, prevDoorClosedState = 0; // state to track if door is open or not
...
void loop() {
  ...
  lightLevel = 1024 - readLightLevel(ldrPin); // invert logic
  char temp1[20];
  ...
  delay(1);
  if ( millis() - doorSamplerTimer > timer2 ||  // check door state every timer2 seconds, OR
      doorClosedState != prevDoorClosedState) { //  when state changes
    char temp3[3];
    sprintf(temp3, "%d", doorClosedState);
    jsonObj1[0] = '\0';
    constructJSONObj("door", temp3, jsonObj1);
    sendDataOverSerial(jsonObj1);
    doorSamplerTimer = millis();
    if( prevDoorClosedState != doorClosedState) 
      beepBuzzer();
    prevDoorClosedState = doorClosedState;
  }
  ...
  // heartbeat LED
  if ( millis() - heartbeatTimer > 5000 ) { // heartbeat every 5 seconds
    blinkLED(ledPin);
