...
float temperature, lightLevel, prevTemperature = 0.0, prevLightLevel = 0.0;
...
void loop() {
  ...
  if ( millis() - temperatureSamplerTimer > timer2 ||
       abs(temperature - prevTemperature)  > 1 ) {
    temp[0] = '\0';
  ...
  if ( millis() - lightSamplerTimer > timer2 ||
       abs(lightLevel - prevLightLevel) > 100) {
    jsonObj1[0] = '\0';
  ...
  if ( millis() - motionSamplerTimer > timer2 ||
    motionLevel != prevMotionLevel ) {
    char temp2[3];
